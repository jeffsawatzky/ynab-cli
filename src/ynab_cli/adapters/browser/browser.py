...
class Browser:
    ...
    def __init__(
        self, default_host: str, host_locale_info: HostLocaleInfo, client_session: AsyncClient | None = None
    ) -> None:
        if client_session is None:
            cookiejar = MozillaCookieJar(filename=f".{default_host}.cookies.txt")
            client_session = AsyncClient(cookies=cookiejar)
    ...
    async def __aenter__(self) -> "Browser":
        if isinstance(self._client_session.cookies.jar, FileCookieJar):
            try:
                self._client_session.cookies.jar.load()
        ...
        response = await self._client_session.get(URL(f"{DEFAULT_SCHEME}://{self._default_host}"))
        log.debug(f"Initial request: {response.status_code}")
    ...
    async def __aexit__(
        ...
        if isinstance(self._client_session.cookies.jar, FileCookieJar):
            try:
                self._client_session.cookies.jar.save()
            except Exception as e:
    ...
    def has_cookies(self, host: str, cookie_names: list[str], scheme: str = DEFAULT_SCHEME) -> bool:
        def find_cookie(cookie_name: str) -> bool:
            for cookie in self._client_session.cookies.jar:
    ...
    def load_cookies(self, cookies_path: Path) -> None:
        if isinstance(self._client_session.cookies.jar, FileCookieJar):
            try:
                self._client_session.cookies.jar.load(str(cookies_path.resolve()))
