...
class BaseScraper(Scraper[M]):
    ...
    def get_locale_info(self, host: str) -> locale.LocaleInfo:
    ...
    def absolute_url(self, response_url: URL, url: str) -> str:
        ...
        if not u.absolute or u.scheme == "":
            if not response_url.absolute or response_url.scheme == "":
...
class BaseCaptchaFormScraper(BaseFormScraper):
    FORM_SELECTOR: ClassVar[str] = ""
    SUBMIT_SELECTOR: ClassVar[str] = ""
    CAPTCHA_SELECTOR: ClassVar[str] = ""
    CAPTCHA_FIELD: ClassVar[str] = ""
    ...
    async def scrape(self, response_url: URL, html: Tag) -> Form | None:
        ...
        img_tag = await self._find_img_tag(html)
        if not img_tag:
            log.warning("No Captcha image found")
            return None
        data = await self._parse_img_tag(response_url, img_tag, data)

        return Form(action=action, method=method, data=data)

    async def _find_img_tag(self, html: Tag) -> Tag | None:
        img_tag = html.select_one(self.__class__.CAPTCHA_SELECTOR)
        if not img_tag:
            return None
        return img_tag

    async def _parse_img_tag(self, response_url: URL, img_tag: Tag, data: dict[str, str]) -> dict[str, str]:
        img_str = self.absolute_url(response_url, self.normalized_text(img_tag.get("src")))
        img_url = URL(img_str, encoded=True)

        async with self._browser.fetch("get", img_url) as response:
            img_bytes = await response.read()

        img_buffer = BytesIO(img_bytes)

        await self._io.display_image(img_buffer)
        captcha_response = await self._io.prompt("Please enter the Captcha")

        data[self.__class__.CAPTCHA_FIELD] = captcha_response
